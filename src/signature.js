...
//    {props.data.signedByApplicant && !props.data.signedByWitness && <p>Declared at <b>{props.data.location}</b> this <b>{props.data.day}</b> of <b>{props.data.month}</b> <b>{props.data.year}</b> before me, <b>{props.data.witnessName}</b> <b>({props.data.witnessTitle})</b>.
console.log(props);
    return (
    <p className="signatureSection">
        I, <b>{props.data.name}</b> of <b>{props.data.address}</b>, 
        occupation <b>{props.data.occupationStatus}</b> solemnly and sincerely declare that I believe the information
        I have given on this form is true and correct, and I make this solemn declaration conscientiously believing the 
